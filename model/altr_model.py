...
from generate_baseline_metrics import (
    parse_arguments,
    create_env_config,
    create_trainer_config,
)
from model.utils import embed_obs_in_map, load_edge_dictionary
from graph_skirmish.data.file_manager import set_visibility
...
from attention_routing.reinforce_baselines import (
    NoBaseline,
    ExponentialBaseline,
    CriticBaseline,
    RolloutBaseline,
...
def optimize(
    ...
    bl_val, bl_loss = baseline.eval(
        attention_input, model_cost
    )  # if bl_val is None else (bl_val, 0) # critic loss
