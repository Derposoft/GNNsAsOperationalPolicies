...
class AltrPolicy(TMv2.TorchModelV2, nn.Module):
    def __init__(
        ...
        prev_layer_size = (
            hiddens[-1] if self._value_branch_separate else self.map.get_graph_size()
        )
    ...
    def forward(
        ...
        _, _, log_ps = self.attention(
            attention_input,
            edges=self.acs_edges_dict,
        ...
        if self._logits:
            ...
            for i in range(len(agent_nodes)):
                ...
                look_action = 1  # TODO!!!!!!!!
                action = Figure8SquadRLLib.convert_multidiscrete_action_to_discrete(
