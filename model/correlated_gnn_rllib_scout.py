...
class CorrelatedGNNScoutPolicy(TMv2.TorchModelV2, nn.Module):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.move_edge_classifier_1 = SlimFC(in_size=self.N_HEADS*self.HIDDEN_DIM*2 + 232, out_size=self.hidden_size, initializer=normc_initializer(1.0),
                                      activation_fn=activation)
        self.move_edge_classifier_2 = SlimFC(in_size=self.hidden_size, out_size=self.hidden_size, initializer=normc_initializer(1.0),
                                        activation_fn=activation)
        self.move_edge_classifier_3 = SlimFC(in_size=self.hidden_size, out_size=1, initializer=normc_initializer(1.0))
    ...
    def forward(
        ...
        self.traditional_features = self.aggregator(x, self.adjacency, agent_nodes = agent_nodes_new)
        if self.is_hybrid:
            self.traditional_features = self._hiddens(torch.cat([self.traditional_features, obs], dim = 1))
            self.traditional_features = self._logits(self.traditional_features)
        ...
        for batch_id in range(len(total_edges)):
            ...
            logits[batch_id][5:11] = self.traditional_features[batch_id]

            for edge_id in range(len(total_edges[batch_id])):
                
                move_source = self._features[batch_id][total_edges[batch_id][edge_id][0]]
                move_target = self._features[batch_id][total_edges[batch_id][edge_id][1]]

                move_feats = torch.cat((move_source, move_target, obs[batch_id])).to(current_device)
