...
def generate_picture(
    ...
    if color_decay:
        ...
        red_bds = np.append(
            [0],
            np.linspace(
                HP_red - TR_red + HP_offset, HP_red + HP_offset, num=len(col_map_red)
            ),
        )
        red_norm = colors.BoundaryNorm(boundaries=red_bds, ncolors=len(col_map_red))
        blue_bds = np.append(
            [0],
            np.linspace(HP_blue - TR_blue, HP_blue + HP_offset, num=len(col_map_blue)),
        )
    ...
    for i, line in enumerate(lines):
        ...
        plt.legend(
            legend_text,
            bbox_to_anchor=(0.07, 0.95, 0.83, 0.1),
            loc="lower left",
            prop={"size": 8},
            mode="expand",
            borderaxespad=0.0,
...
def local_run(
    ...
    for file in os.listdir(directory):
        ...
        if log_file.endswith(".txt") and log_file.startswith(prefix):
            if route_only:
                fig_folder = generate_picture_route(
                    env_dir, log_dir, log_file, route_info
                )
                pause_frame = 0
            else:
                fig_folder, pause_frame = generate_picture(
                    env_dir,
                    log_dir,
                    log_file,
                    HP_red,
            ...
            frame_add_background(
                fig_folder,
                os.path.join(log_dir, f"{log_file[:-4]}.gif"),
                bg_pic,
                fps,
                pause_frame,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env_dir", type=str, default="../../", help="path to project root"
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default="../../logs/visuals/demo/",
        help="path to log file folder",
    )
    parser.add_argument("--prefix", type=str, default="log_", help="log file prefix")
    parser.add_argument(
        "--background", type=str, default="../../logs/visuals/background.png"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="stop animation if agent is dead",
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="gradient colors for HP",
    )

    parser.add_argument("--route_only", type=bool, default=False)  # exclude step info
    parser.add_argument(
        "--route_info", type=str, default="name"
    )  # choose from ['name', 'pos', 'idx']
