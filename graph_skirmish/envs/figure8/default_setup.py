...
from graph_skirmish.data.data_helper import (
    get_node_name_from_pos_abs,
    get_pos_norms,
    check_pos_abs_range,
)
...
INIT_CONFIGS = {
    ...
    "init_blue": None,
    "init_health_red": 10,
    "init_health_blue": 100,
    "damage_maximum": 5,
    "fixed_start": -1,
}
INIT_CONFIGS_LOCAL = [
    "threshold_damage_2_red",
    "threshold_damage_2_blue",
    "act_masked",
    "penalty_stay",
]
        ...
        if configs is None:
            ...
            for idx, _config in enumerate(configs):
                ...
                if isinstance(_pos, tuple):
                    assert check_pos_abs_range(
                        _pos
                    ), "Pos tuple: {} out of range".format(_pos)
                elif isinstance(_pos, str):
                    assert (
                        _pos in INIT_REGION
                    ), "Invalid region:'{}' (not in {})".format(
                        _pos, INIT_REGION.keys()
                    )
...
def get_state_shapes(n_G, n_R, n_B, shape_tokens) -> int:
    ...
    state_self = (
        (bit_embed if _is_embed else n_G)
        + _has_self_dir * look_dir
        + (_has_sight + _has_range) * n_B
    )
    state_B = (
        ((bit_embed + look_dir) * n_B) if _is_embed else (n_G + look_dir * n_B)
    ) + (_has_sight + _has_range) * n_B
