...
from .rewards.rewards_simple import (
    get_step_engage,
    get_step_overlay,
    get_episode_reward_agent,
)
...
class Figure8Squad(gym.Env):
    def __init__(self, max_step=40, n_red=2, n_blue=1, **kwargs):
        ...
        # init spaces
        self.__action_space = ActionSpaces(
            [
                spaces.MultiDiscrete([len(local_action_move), len(local_action_turn)])
                for _ in range(len(self.learning_agent))
            ]
        )
        # <default>: all agents have identical observation shape
        self.state_shape = env_setup.get_state_shape(
            self.map.get_graph_size(), n_red, n_blue, self.obs_token
        )
        # agent obs size: [self: pos(6or27)+dir(4)+flags(2*B) +teamB: pos+next_move(4)+flags(2*B) +teamR: 6*(n_R-1)or27]
        self.__observation_space = ObservationSpaces(
            [
                spaces.Box(low=0, high=1, shape=(self.state_shape,), dtype=np.int8)
                for _ in range(len(self.learning_agent))
            ]
        )
    ...
    def step(self, n_actions):
        assert len(n_actions) == len(
            self.learning_agent
        ), f"[EnvError] Invalid action shape {n_actions}"
        ...
        # get rewards
        n_reward = self._step_rewards(
            action_penalty_red, R_engage_B, B_engage_R, R_overlay
        )
    ...
    def _take_action_red(self, n_actions):
        ...
        _action_stay_penalty = (
            self.configs["penalty_stay"] if "penalty_stay" in self.configs else 0
        )

        for agent_i, actions in enumerate(n_actions):
            # check input action if in range of the desired discrete action space
            assert self.__action_space[agent_i].contains(
                actions
            ), f"{actions}: action out of range"
            ...
            prev_node, list_neighbors, list_acts = self.map.get_all_states_by_node(
                agent_encoding
            )
            ...
            # make 'Move' and then 'Turn' actions
                ...
                if action_turn:
                    _dir = (
                        env_setup.act.TURN_L[action_move]
                        if action_turn == 1
                        else env_setup.act.TURN_R[action_move]
                    )
    ...
    def _take_action_blue(self, n_actions=None):
        for agent_i in range(self.num_blue):
            ...
            _idx = (self.step_counter + self.blue_offsets[agent_i]) % self.routes[
                _route
            ].get_route_length()
    ...
    def _update(self):
        ...
        if self.invalid_masked:
            self.action_mask = [
                np.zeros(sum(tuple(self.__action_space[_].nvec)), dtype=np.bool_)
                for _ in range(len(self.learning_agent))
            ]
        for _r in range(self.num_red):
            ...
            for _b in range(self.num_blue):
                ...
                R_see_B[_r, _b] = self.is_in_sight(
                    node_r, node_b, self.team_red[_r].agent_dir
                )
                R_engage_B[_r, _b] = self.is_in_range(
                    node_r, node_b, self.team_red[_r].agent_dir
                )
                B_see_R[_b, _r] = self.is_in_sight(
                    node_b, node_r, self.team_blue[_b].agent_dir
                )
                B_engage_R[_b, _r] = self.is_in_range(
                    node_b, node_r, self.team_blue[_b].agent_dir
                )
            ...
            if self.invalid_masked:
                ...
                valid = set(
                    self.map.get_actions_by_node(self.team_red[_r].get_encoding()) + [0]
                )
        ...
        if self.obs_token["obs_embed"]:
            ...
            for agent_i in range(self.num_red):
                _state_R_embed.append(
                    get_emb_from_name(self.team_red[agent_i].get_encoding())
                )
            _state_B_embed = []
            for agent_i in range(self.num_blue):
                _state_B_embed.append(
                    get_emb_from_name(self.team_blue[agent_i].get_encoding())
                )
    ...
    def agent_interaction(self, R_engage_B, B_engage_R):
        ...
        for _b in range(self.num_blue):
            ...
            if self.team_blue[_b].get_end_step() > 0:
                continue
            _damage_taken_blue = (
                self.configs["init_health_blue"] - self.team_blue[_b].get_health()
            )
    ...
    def _step_rewards(self, penalties, R_engage_B, B_engage_R, R_overlay):
        ...
        for agent_r in range(self.num_red):
            rewards[agent_r] += get_step_overlay(
                R_overlay[agent_r], **self.rewards["step"]
            )
            for agent_b in range(self.num_blue):
                rewards[agent_r] += get_step_engage(
                    r_engages_b=R_engage_B[agent_r, agent_b],
    ...
    def _episode_rewards(self):
        ...
        _health_lost_r = [
            _HP_full_r - self.team_red[_r].get_health() for _r in range(self.num_red)
        ]
        _damage_cost_r = [
            self.team_red[_r].damage_total() for _r in range(self.num_red)
        ]
        _health_lost_b = [
            _HP_full_b - self.team_blue[_b].get_health() for _b in range(self.num_blue)
        ]
        ...
        for agent_r in range(self.num_red):
            for agent_b in range(self.num_blue):
                rewards[agent_r] += get_episode_reward_agent(
                    _health_lost_r[agent_r],
                    _health_lost_b[agent_b],
                    _threshold_r,
                    _threshold_b,
                    _damage_cost_r[agent_r],
                    _end_step_b[agent_b],
    ...
    def _init_env_config(self, **kwargs):
        ...
        if self.invalid_masked is False:
            self.configs["penalty_invalid"] = env_setup.ACT_MASKED[
                "unmasked_invalid_action_penalty"
            ]

        # check init_red init configs, must have attribute "pos":[str/tuple] for resetting agents
        self.configs["init_red"] = env_setup.check_agent_init(
            "red", self.num_red, self.configs["init_red"]
        )
        # check init_blue, must have attribute "route":[str] used in loading graph files. default: '0'
        self.configs["init_blue"] = env_setup.check_agent_init(
            "blue", self.num_blue, self.configs["init_blue"]
        )
        # get all unique routes. (blue agents might share patrol route)
        self.configs["route_lookup"] = list(
            set(_blue["route"] for _blue in self.configs["init_blue"])
        )
    ...
    def _load_map_data(self):
        # load graphs
        self.map, self.routes = load_graph_files(
            env_path=self.configs["env_path"],
        ...
        for _route in range(len(self.routes)):
            ...
            for _idx in range(route_len):
                self.routes[_route].list_node[_idx] = self.map.get_index_by_name(
                    self.routes[_route].list_code[_idx]
                )
            list_dir = [0] * (route_len - 1)
            for _idx in range(route_len - 1):
                list_dir[_idx] = self.map.get_edge_attr_acs_by_idx(
                    self.routes[_route].list_node[_idx],
                    self.routes[_route].list_node[_idx + 1],
                )
            off_dir = self.map.get_edge_attr_acs_by_idx(
                self.routes[_route].list_node[-1], self.routes[_route].list_node[0]
            )
    ...
    def _init_agents(self):
        ...
        for idx, init_blue in enumerate(self.configs["init_blue"]):
            ...
            b_route = self.configs["route_lookup"].index(
                init_blue["route"]
            )  # int: index in the route lookup list
    ...
    def _reset_agents(self):
        ...
        for idx, init_red in enumerate(self.configs["init_red"]):
            ...
            self.team_red[idx].reset(
                _node=r_node, _code=r_code, _dir=r_dir, _health=HP_red
            )
